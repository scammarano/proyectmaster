...
if (is_post()) {
  ...
  if ($action==='save_article') {
    ...
    $type=trim(post_param('article_type',''));
    $modules=(int)post_param('modules',1);
    $req = (int)post_param('requires_cover',0);

    if($code===''||$name===''||$type===''){ set_flash('error','Código, nombre y tipo son requeridos'); redirect('index.php?page=articles'); }

    if($id>0){
      if($HAS_REQ){
        db_query("UPDATE articles SET code=?, name=?, brand_id=?, series_id=?, article_type=?, modules=?, requires_cover=? WHERE id=?",
          [$code,$name,$brand_id,$series_id,$type,$modules,$req,$id]);
      } else {
        db_query("UPDATE articles SET code=?, name=?, brand_id=?, series_id=?, article_type=?, modules=? WHERE id=?",
          [$code,$name,$brand_id,$series_id,$type,$modules,$id]);
      }
      if(function_exists('user_log')) user_log('article_update','article',$id,$code);
      set_flash('success','Artículo actualizado.');
    } else {
      if($HAS_REQ){
        db_query("INSERT INTO articles(code,name,brand_id,series_id,article_type,modules,requires_cover) VALUES (?,?,?,?,?,?,?)",
          [$code,$name,$brand_id,$series_id,$type,$modules,$req]);
      } else {
        db_query("INSERT INTO articles(code,name,brand_id,series_id,article_type,modules) VALUES (?,?,?,?,?,?)",
          [$code,$name,$brand_id,$series_id,$type,$modules]);
      }
...
$rows = db_query("
  SELECT a.*, b.name brand_name, s.name series_name
  FROM articles a
      ...
      <div class="card-body">
        <form method="post" class="row g-2">
          ...
          <div class="col-6">
            ...
            <input class="form-control" type="number" name="modules" id="a_modules" value="1" min="0">
          </div>
          <div class="col-12">
          ...
          <div class="col-6">
            ...
            <select class="form-select" name="article_type" id="a_type" required>
          ...
          <div class="col-6">
            ...
            <select class="form-select" name="requires_cover" id="a_req">
          ...
          <div class="col-12">
            ...
            <?php if(!$HAS_DIVMAP): ?>
              <div class="text-danger small">Falta tabla <code>article_divisions</code>. Ejecuta el patch de puntos (article_divisions).</div>
            <?php else: ?>
              ...
              <div class="form-text">Un artículo puede pertenecer a varias divisiones.</div>
            <?php endif; ?>
          </div>
              ...
              <tr>
                ...
                <td class="text-end">
                  <button class="btn btn-sm btn-outline-primary" type="button"
                    onclick='fillArticle(<?= (int)$r["id"] ?>, <?= json_encode($r["code"]) ?>, <?= json_encode($r["name"]) ?>, <?= json_encode($r["article_type"]) ?>, <?= (int)$r["modules"] ?>, <?= json_encode((string)($r["brand_id"]??"")) ?>, <?= json_encode((string)($r["series_id"]??"")) ?>, <?= (int)($r["requires_cover"]??0) ?>, <?= json_encode($div_ids) ?>)'>
