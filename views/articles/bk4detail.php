...
$typeFieldRules = rules_articles_type_field_rules();
$defaultRule = rules_articles_default();
...
if ($id > 0) {
  ...
  if ($HAS_TYPES && $HAS_ATID) {
    $tc = db_query("SELECT code FROM article_types WHERE id=?", [(int)$article['article_type_id']])->fetch();
    $type_code = $tc['code'] ?? '';
  }
...
function duplicate_article(int $id, bool $HAS_MAP): int {
...
if (is_post()) {
  ...
  if ($action === 'duplicate' && $id > 0) {
    $newId = duplicate_article($id, $HAS_MAP);
...
// Mapeo para UI: type_code -> show/hide (desde archivo)
$jsRules = [];
foreach($types as $t){
  $c = (string)$t['code'];
  $rule = $typeFieldRules[$c] ?? $defaultRule;
  $jsRules[$c] = $rule;
}
?>
  ...
  <div class="d-flex gap-2">
    ...
    <?php if($id>0): ?>
      <form method="post" class="d-inline" onsubmit="return confirm('¿Duplicar este artículo?');">
        ...
        <button class="btn btn-sm btn-outline-warning"><i class="bi bi-copy"></i> Duplicar</button>
      </form>
    <?php endif; ?>
  </div>
</div>
  ...
  <div class="card-body">
    <form method="post" action="index.php?page=article_detail<?= $id>0 ? '&id='.h($id) : '' ?>" class="row g-3" id="articleSaveForm">
      ...
      <div class="col-md-3" id="wrap_cover">
        ...
        <select class="form-select" name="requires_cover" id="requires_cover">
...
(function(){
  ...
  // Reglas de UI basadas en archivo /config/rules.php (no heurística)
  const rules = <?= json_encode($jsRules, JSON_UNESCAPED_UNICODE) ?>;
  const fallback = <?= json_encode($defaultRule, JSON_UNESCAPED_UNICODE) ?>;

  const typeSel = document.getElementById('article_type_id');
  const wrapM = document.getElementById('wrap_modules');
  ...
  function applyTypeUI(){
    ...
    const r = rules[code] || fallback;
    const show = new Set((r.show||[]).map(x=>String(x)));
    const hide = new Set((r.hide||[]).map(x=>String(x)));

    if(wrapM){
      wrapM.style.display = (hide.has('modules') ? 'none' : '');
      if(show.has('modules')) wrapM.style.display = '';
    }
    if(wrapC){
      wrapC.style.display = (hide.has('requires_cover') ? 'none' : '');
      if(show.has('requires_cover')) wrapC.style.display = '';
    }
  }
