...
function render_area($area, $by, $level=0, $closed=false) {
  $id=(int)$area['id'];
  $kids=$by[$id] ?? [];
  ...
  echo '<span class="badge text-bg-light border" title="Arrastra para mover" style="cursor:'.($closed?'not-allowed':'grab').'"><i class="bi bi-grip-vertical"></i></span>';
  ...
  if ($hasKids) {
    ...
    foreach ($kids as $k) render_area($k,$by,$level+1,$closed);
    echo '</div>';
  }
}
...
function area_is_descendant(int $candidateParentId, int $movingId, int $project_id): bool {
  // sube por la cadena parent_area_id
  $cur = $candidateParentId;
  $guard = 0;
  while ($cur > 0 && $guard < 5000) {
    if ($cur === $movingId) return true;
    $row = db_query("SELECT parent_area_id FROM areas WHERE id=? AND project_id=?", [$cur, $project_id])->fetch();
    if (!$row) return false;
    $cur = (int)($row['parent_area_id'] ?? 0);
    $guard++;
  }
  return false;
}
...
if (is_post()) {
  ...
  if ($action==='div_create' && $HAS_DIVS) {
    $dname = trim(post_param('d_name',''));
    $dprefix = trim(post_param('d_prefix',''));
    if ($dname==='') { add_flash('error','Nombre de división requerido'); redirect('index.php?page=project_detail&id='.$project_id.'#tab-general'); }

    if ($HAS_DIV_PREFIX) {
      db_query("INSERT INTO divisions(name,prefix) VALUES (?,?)", [$dname, $dprefix]);
    } else {
      db_query("INSERT INTO divisions(name) VALUES (?)", [$dname]);
    }
    $newDid = (int)db_connect()->lastInsertId();

    if ($HAS_PDIV) db_query("INSERT IGNORE INTO project_divisions(project_id,division_id) VALUES (?,?)", [$project_id,$newDid]);

    add_flash('success','División creada'.($HAS_PDIV?' y agregada al proyecto.':'.'));
    redirect('index.php?page=project_detail&id='.$project_id.'#tab-general');
  }
  ...
  if ($action==='move_area') {
    if (!$HAS_PARENT) { add_flash('error','Tu tabla areas no soporta jerarquía (parent_area_id).'); redirect('index.php?page=project_detail&id='.$project_id.'#tab-areas'); }
    $moving = (int)post_param('area_id',0);
    $newParentRaw = post_param('new_parent_id','');
    $newParent = ($newParentRaw==='' || $newParentRaw==='0') ? null : (int)$newParentRaw;

    $exists = db_query("SELECT id FROM areas WHERE id=? AND project_id=?", [$moving,$project_id])->fetch();
    if (!$exists) { add_flash('error','Área inválida.'); redirect('index.php?page=project_detail&id='.$project_id.'#tab-areas'); }

    if ($newParent !== null) {
      if ($newParent === $moving) { add_flash('error','No puedes mover un área dentro de sí misma.'); redirect('index.php?page=project_detail&id='.$project_id.'#tab-areas'); }
      $pExists = db_query("SELECT id FROM areas WHERE id=? AND project_id=?", [$newParent,$project_id])->fetch();
      if (!$pExists) { add_flash('error','Área padre inválida.'); redirect('index.php?page=project_detail&id='.$project_id.'#tab-areas'); }
      if (area_is_descendant($newParent, $moving, $project_id)) {
        add_flash('error','Movimiento inválido: crearía un ciclo (mover dentro de un hijo).');
        redirect('index.php?page=project_detail&id='.$project_id.'#tab-areas');
      }
    }

    db_query("UPDATE areas SET parent_area_id=? WHERE id=? AND project_id=?", [$newParent, $moving, $project_id]);
...
<div class="tab-content border border-top-0 p-3">
  ...
  <div class="tab-pane fade show active" id="tab-general">
    <!-- (sin cambios aquí) -->
    <?php /* Mantengo tu contenido tal cual; por brevedad, no lo toco */ ?>
    <div class="row g-3">
      <div class="col-lg-6">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <?php else: ?>
              ...
              <div class="form-text mt-2">
                <i class="bi bi-info-circle"></i> Puedes <b>arrastrar</b> un área encima de otra para convertirla en subárea. Arrastra al borde superior (espacio vacío) para dejarla en raíz.
              </div>
            <?php endif; ?>
          </div>
        </div>
      ...
      <div class="col-lg-7">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <?php else: ?>
              <div id="areas-root-drop" class="border border-dashed rounded p-2 mb-2 text-muted small" style="border-style:dashed; display:<?= $closed?'none':'block' ?>;">
                Arrastra aquí para dejar el área en <b>raíz</b>.
              </div>
              <?php foreach($roots as $r) render_area($r,$by,0,$closed); ?>
            <?php endif; ?>
          </div>
        </div>

        <?php if(!$closed && $HAS_PARENT): ?>
          <form method="post" id="moveAreaForm" class="d-none">
            <input type="hidden" name="action" value="move_area">
            <input type="hidden" name="area_id" id="mv_area_id" value="">
            <input type="hidden" name="new_parent_id" id="mv_parent_id" value="">
          </form>
        <?php endif; ?>
...
// ---- Drag & Drop (HTML5) para mover áreas ----
(function(){
  const closed = <?= $closed ? 'true':'false' ?>;
  const hasParent = <?= $HAS_PARENT ? 'true':'false' ?>;
  if (closed || !hasParent) return;

  let dragId = null;

  function submitMove(areaId, newParentId){
    const f = document.getElementById('moveAreaForm');
    if(!f) return;
    document.getElementById('mv_area_id').value = areaId;
    document.getElementById('mv_parent_id').value = (newParentId===null ? '' : String(newParentId));
    f.submit();
  }

  document.querySelectorAll('.area-row').forEach(el=>{
    el.addEventListener('dragstart', (e)=>{
      dragId = el.getAttribute('data-area-id');
    ...
    el.addEventListener('drop', (e)=>{
      ...
      const target = el.getAttribute('data-area-id');
      if(!src || !target) return;
      if(src===target) return;
      submitMove(src, target);
    });
  });

  const rootDrop = document.getElementById('areas-root-drop');
  if(rootDrop){
    rootDrop.addEventListener('dragover', (e)=>{ e.preventDefault(); rootDrop.classList.add('border-primary'); });
    rootDrop.addEventListener('dragleave', ()=>{ rootDrop.classList.remove('border-primary'); });
