...
function render_area($area, $by, $directMap, $level=0) {
  ...
  if ($hasKids) {
    echo '<div class="collapse show ms-2 ps-3 border-start" id="'.$cid.'">';
...
function duplicate_area_recursive(int $old_area_id, int $project_id, ?int $new_parent_id): int {
  $old = db_query("SELECT * FROM areas WHERE id=? AND project_id=?", [$old_area_id, $project_id])->fetch();
  if (!$old) return 0;
  ...
  if ($HAS_AREA_CREATED_AT) {
    db_query("INSERT INTO areas(project_id,".($HAS_PARENT?"parent_area_id,":"")."name,created_at) VALUES (?,?,?,NOW())",
      $HAS_PARENT ? [$project_id, $new_parent_id, ($old['name'] ?? '').' (Copia)'] : [$project_id, ($old['name'] ?? '').' (Copia)']);
  } else {
    db_query("INSERT INTO areas(project_id,".($HAS_PARENT?"parent_area_id,":"")."name) VALUES (?,?,?)",
      $HAS_PARENT ? [$project_id, $new_parent_id, ($old['name'] ?? '').' (Copia)'] : [$project_id, ($old['name'] ?? '').' (Copia)']);
  }
  $new_id = (int)db_connect()->lastInsertId();

  // duplicar puntos del área (mantiene project_id, actualiza area_id)
  if (table_exists('points')) {
    $pts = db_query("SELECT * FROM points WHERE area_id=?".(col_exists('points','project_id')?" AND project_id=?":""), 
      col_exists('points','project_id')?[$old_area_id,$project_id]:[$old_area_id])->fetchAll();
    if ($pts) {
  ...
  // hijos
  if ($HAS_PARENT) {
    $kids = db_query("SELECT id FROM areas WHERE parent_area_id=? AND project_id=? ORDER BY id", [$old_area_id, $project_id])->fetchAll();
...
if (is_post()) {
  ...
  if ($action==='add_area') {
    ...
    if ($name==='') { add_flash('error','Nombre requerido'); header('Location: index.php?page=project_detail&id='.$project_id.'#tab-areas');
    exit;
  }
    ...
    header('Location: index.php?page=project_detail&id='.$project_id.'#tab-areas');
    exit;
  }
  ...
  if ($action==='duplicate_area') {
    $aid = (int)post_param('area_id',0);
    if ($aid<=0) { add_flash('error','Área inválida'); header('Location: index.php?page=project_detail&id='.$project_id.'#tab-areas'); exit; }

    $old = db_query("SELECT id, parent_area_id FROM areas WHERE id=? AND project_id=?", [$aid,$project_id])->fetch();
    if (!$old) { add_flash('error','Área no encontrada'); header('Location: index.php?page=project_detail&id='.$project_id.'#tab-areas'); exit; }

    $newParent = null;
    if (col_exists('areas','parent_area_id')) {
      $newParent = ($old['parent_area_id'] ?? null);
      $newParent = ($newParent===null || (int)$newParent===0) ? null : (int)$newParent;
    }
    duplicate_area_recursive($aid, $project_id, $newParent);
