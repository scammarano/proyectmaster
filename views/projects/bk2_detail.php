...
$divisions = [];
if ($hasDivisions) {
  // code_prefix es opcional; si no existe no rompe
...
if ($hasAreaDivisions) {
  ...
  ", [$project_id])->fetchAll();
  foreach ($rows as $r) {
    if (!isset($areaDivMap[$r['area_id']])) $areaDivMap[$r['area_id']] = [];
    if (!empty($r['division_id'])) $areaDivMap[$r['area_id']][] = (int)$r['division_id'];
  }
}
...
if ($hasAreas && $hasAreaDivisions && $hasDivisions && $hasProjectDivisions) {
  ...
  // Fallback: si no hay project_divisions, muestra todas las divisiones sin áreas
  if ($hasDivisions) {
...
// Conteo de puntos por división (si points.division_id existe)
$pointsByDivision = [];
if ($hasPoints && $hasPointsDivisionId) {
  $pointsByDivision = db_query("
...
<div class="row g-3">
  <div class="col-lg-4">
    ...
    <?php if ($pointsByDivision): ?>
      <div class="card">
        <div class="card-header">Puntos por división (proyecto)</div>
        <div class="card-body small">
          <?php foreach ($pointsByDivision as $r): ?>
            <div class="d-flex justify-content-between">
              <span>División ID <?= h($r['division_id']) ?></span>
              <strong><?= h($r['cnt']) ?></strong>
            </div>
          <?php endforeach; ?>
          <div class="text-muted mt-2">
            * Esto mejora cuando el punto tenga división asignada y joins con nombre/prefijo.
          </div>
        </div>
      </div>
    <?php endif; ?>
        ...
        <table class="table table-sm table-striped align-middle mb-0">
          <thead>
            <tr>
              ...
              <th style="width: 320px;">Editar divisiones</th>
              ...
              <td>
                ...
                <?php else: ?>
                  <form method="post" class="d-flex gap-2">
                    ...
                    <button class="btn btn-sm btn-primary" title="Guardar">
          ...
          <?php endforeach; ?>
          <?php if (!$areas): ?>
            <tr><td colspan="5" class="text-muted text-center py-3">No hay áreas.</td></tr>
          <?php endif; ?>
          </tbody>
        </table>
      </div>
