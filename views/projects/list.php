...
$HAS_ATTACH = table_exists('attachments') && col_exists('attachments','entity_type') && col_exists('attachments','entity_id');
$HAS_PDIV = table_exists('project_divisions') && table_exists('divisions');

function project_has_any_children(int $pid, bool $hasAttach): bool {
  ...
  $att = 0;
  if ($hasAttach) $att = (int)db_query("SELECT COUNT(*) c FROM attachments WHERE entity_type='project' AND entity_id=?",[$pid])->fetch()['c'];
  return ($a+$p+$att)>0;
}
...
if (is_post()) {
  ...
  if ($action === 'duplicate_project') {
    ...
    // duplicar áreas (incluye jerarquía si existe parent_area_id)
    ...
    $map = [];
    foreach ($areas as $a) {
      if ($HAS_AREA_CREATED_AT) db_query("INSERT INTO areas(project_id,parent_area_id,name,created_at) VALUES (?,?,?,NOW())", [$newProjectId, null, $a['name']]);
      else db_query("INSERT INTO areas(project_id,parent_area_id,name) VALUES (?,?,?)", [$newProjectId, null, $a['name']]);
  ...
  if ($action === 'delete_project') {
    ...
    if (project_has_any_children($id, $HAS_ATTACH)) {
...
// Query con divisiones (si existen)
if ($HAS_PDIV) {
  $rows = db_query("
    SELECT p.*,
      ...
      (
        SELECT GROUP_CONCAT(CONCAT(COALESCE(d.prefix,''),IF(d.prefix IS NULL OR d.prefix='', '', ':'),d.name) ORDER BY d.name SEPARATOR ' | ')
...
<script>
setCtx('Proyectos', `
  <a class="btn btn-sm btn-primary" href="index.php?page=project_new"><i class="bi bi-plus-circle"></i> Nuevo</a>
`);
</script>

<div class="d-flex justify-content-between align-items-center mb-3">
  <h1 class="h3 mb-0"><i class="bi bi-kanban"></i> Proyectos</h1>
  <a class="btn btn-primary" href="index.php?page=project_new"><i class="bi bi-plus-circle"></i> Nuevo proyecto</a>
</div>
    ...
    <table class="table table-striped align-middle mb-0">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <?php if($HAS_PDIV): ?>
              <span class="text-muted small"><?= h($p['divisions_txt'] ?? '') ?: '—' ?></span>
            <?php else: ?>
        ...
        <?php endforeach; ?>
        <?php if(!$rows): ?><tr><td colspan="7" class="text-muted text-center py-4">No hay proyectos.</td></tr><?php endif; ?>
      </tbody>
    </table>
  </div>
</div>
