...
if (is_post()) {
  ...
  if ($action === 'duplicate_project') {
    ...
    $created_by = function_exists('current_user') && current_user() ? (int)current_user()['id'] : null;

    if ($HAS_CLOSED) {
      db_query("INSERT INTO projects(name,is_closed,created_by,created_at) VALUES (?,?,?,NOW())", [$name, 0, $created_by]);
    } else {
      db_query("INSERT INTO projects(name,created_by,created_at) VALUES (?,?,NOW())", [$name, $created_by]);
    }
...
?>
<script>
setCtx('Proyectos', `<a class="btn btn-sm btn-primary" href="index.php?page=projects&action=new"><i class="bi bi-plus-circle"></i> Nuevo</a>`);
</script>
  ...
  <div class="table-responsive">
    <table class="table table-striped align-middle mb-0">
      ...
      <tbody>
        ...
        <tr>
          <td><a class="fw-semibold text-decoration-none" href="index.php?page=project_detail&id=<?=h($p['id'])?>"><?=h($p['name'])?></a></td>
          ...
          <td class="text-end">
            ...
            <form method="post" class="d-inline" onsubmit="return confirm('¿Duplicar proyecto? Se duplican áreas (incl. subáreas) y puntos.');">
